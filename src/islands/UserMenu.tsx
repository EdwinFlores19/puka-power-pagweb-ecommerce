    ...
    <>
      ...
      <Show when={showLoginModal()}>
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div
            ref={modalRef}
            class="relative w-full max-w-md bg-brand-cream rounded-3xl shadow-2xl border border-brand-wine/10 overflow-hidden animate-bubble-in"
          >
            {/* Header */}
            <div class="p-6 bg-gradient-to-br from-brand-wine to-brand-wineDark text-brand-cream">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-black text-lg">Inicia sesión</p>
                    <p class="text-xs text-brand-cream/70">Mio el gato guardián te espera</p>
                  </div>
                </div>
                <button
                  onClick={closeLoginModal}
                  aria-label="Cerrar"
                  class="w-8 h-8 rounded-full bg-brand-cream/10 hover:bg-brand-cream/20 flex items-center justify-center text-brand-cream/70 hover:text-brand-cream transition-colors"
                >
            ...
            {/* Form */}
            <form onSubmit={handleEmbeddedLogin} class="p-6 space-y-4">
              <div>
              ...
              <Show when={loginError()}>
                <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-start gap-2">
                  ...
                  <p class="text-xs font-semibold text-red-700 leading-relaxed">{loginError()}</p>
                </div>
              </Show>
              ...
              <button
                ...
                class="w-full py-4 bg-gradient-to-r from-brand-wine to-brand-wineDark hover:from-brand-wineDark hover:to-brand-wine disabled:opacity-60 text-brand-cream font-extrabold uppercase tracking-widest text-sm rounded-xl transition-all shadow-xl shadow-brand-wine/30 flex items-center justify-center gap-2"
              >
                {loginLoading() ? (
                  <>
              ...
              <div class="text-center pt-2 border-t border-brand-primary/10 text-xs space-y-2">
